...
const { getShopProductById } = useShopData();
...
onBeforeMount(async () => {
  event.value = await getShopProductById(String(slugroute), ticker.shopsource);
});
...
function cartAddStore() {
  if (event.value) {
    addToCart({
      id: event.value.id,
      amount: 1,
      image: event.value.images ? event.value.images[0] : "",
      title: event.value.title,
      price: event.value.price,
      shipprice: event.value.shipprice || 0,
      variation: "",
    });
  }
}
